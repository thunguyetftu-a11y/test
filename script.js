...
const details =
[...document.querySelectorAll('.value-dropdown')]
...
function matches(row, filters) { return state.columns.every((column) => { const filter = filters[column]; const value = normalize(row[column]); 
...
document
.querySelectorAll('.value-dropdown')
.forEach(details => {
updateDropdownSummary(details);
});
